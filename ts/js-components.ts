import { moonData } from "./facts";

import { MoonDataInterface } from "./interfaces.js";

export function MoonInfoBlocks(data: MoonDataInterface, error: string = ""): string {
    if (error) {
        return `<p> ${error} </p>`;
    }
    return `
...
export function RandomMoonFactsBlocks(): string {
...
export function FormDialogComponent() {
    return `<form action="/sendComplain" method="post">
